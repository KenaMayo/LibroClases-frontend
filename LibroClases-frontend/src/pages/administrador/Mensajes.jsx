...
export default function Mensajes() {
  ...
  async function cargarDatos() {
    ...
    try {
      ...
      setMensajes(msgs);
      setUsuarios(usrs.filter((u) => u.id !== user?.id));
  ...
  return (
    <div>
          ...
          <form onSubmit={handleEnviar}>
            ...
            <div className="mb-3">
              ...
              <textarea
                ...
                maxLength={500}
              />
              <div className="text-muted small text-end mt-1">{contenido.length}/500</div>
            </div>
      ...
      {/* TODOS LOS MENSAJES (admin ve todo) */}
      <div className="card border-0 shadow-sm">
        ...
        <div className="card-body p-0">
          {mensajes.length === 0 ? (
            <p className="text-muted text-center py-4">No hay mensajes aún.</p>
          ) : (
            <ul className="list-group list-group-flush">
              {mensajes.map((m) => (
                <li key={m.id} className="list-group-item">
                  <div className="d-flex gap-2 mb-1">
                    <span className="badge bg-primary">De: {getNombre(m.emisorId)}</span>
                    <span className="badge bg-secondary">Para: {getNombre(m.receptorId)}</span>
                  </div>
                  <p className="mb-0 text-secondary">{m.contenido}</p>
                </li>
              ))}
            </ul>
          )}
